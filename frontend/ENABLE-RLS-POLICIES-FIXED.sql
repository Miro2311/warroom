...
BEGIN
    ...
    IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'partner_notes') THEN
        ...
        EXECUTE 'CREATE POLICY "Users can delete their own notes"
    ...
    IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'partner_red_flags') THEN
        ...
        EXECUTE 'CREATE POLICY "Users can delete their own red flags"
        ON public.partner_red_flags FOR DELETE
        USING (reported_by_id = auth.uid())';

        -- Partner owners can delete red flags on their partners
        EXECUTE 'CREATE POLICY "Users can delete red flags for their partners"
        ON public.partner_red_flags FOR DELETE
        USING (
          partner_id IN (
            SELECT id FROM public.partners WHERE user_id = auth.uid()
